...
# load data
# **************************************
df_train = read.csv("airbnb/train_users_2.csv")
df_test = read.csv("airbnb/test_users.csv")
labels = df_train[, c('id', 'country_destination')]
df_test$country_destination = NA

age_gender_bkts <- fread("airbnb/age_gender_bkts.csv", data.table=F)
countries <- fread("airbnb/countries.csv", data.table=F)
sample_submission_NDF <- fread("airbnb/sample_submission_NDF.csv", data.table=F)
sessions <- fread("airbnb/sessions.csv", data.table=F)
